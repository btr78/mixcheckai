...
const STRIPE_PAYMENT_LINK = "[https://buy.stripe.com/3cIcN6gBO375e6dbQkgbm03](https://buy.stripe.com/3cIcN6gBO375e6dbQkgbm03)";
...
function Nav({ navigate, page, isPro, onUnlockClick }) {
  ...
  return (
    <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, background: scrolled ? "rgba(7,9,15,0.93)" : "transparent", backdropFilter: scrolled ? "blur(14px)" : "none", borderBottom: scrolled ? "1px solid rgba(255,255,255,0.06)" : "none", padding:"0 20px", transition:"all 0.3s" }}>
      ...
      <style>{`* { box-sizing:border-box; } @keyframes wave { from{transform:scaleY(0.35)} to{transform:scaleY(1)} } @keyframes float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-7px)} } @keyframes fadein { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:translateY(0)} } @keyframes spin { to{transform:rotate(360deg)} } @keyframes glow { 0%,100%{box-shadow:0 0 20px rgba(0,229,160,0.15)} 50%{box-shadow:0 0 40px rgba(0,229,160,0.35)} } html { scroll-behavior:smooth; } @media print { nav, footer, .no-print { display:none !important; } body { background:#fff !important; color:#000 !important; } .print-section { display:block !important; } }`}</style>
    </nav>
  );
}
...
<div class="grid">
  <div class="metric">
    ...
    <div class="metric-value ${lufsD.color === '#00e5a0' ? 'good' : lufsD.color === '#ffb347' ? 'warn' : 'bad'}">${results.lufs} LUFS</div>
